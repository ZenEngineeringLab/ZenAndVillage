...
11. [Condominium Asset Management](#11-condominium-asset-management)
12. [Consumable Inventory Management](#12-consumable-inventory-management)
...
#### Key Domain Communication Rules

- Assembly summons must be sent via the formal channel defined in the bylaws (email + physical notice + bulletin board). Only items listed on the official notice may be voted on.
- Delinquency notices must be directed exclusively to the responsible unit owner — never broadcast in group channels (LGPD and privacy requirements).
- Emergency notices (water, gas, structural failure) must reach all residents simultaneously through every available channel.
- Formal communications (summons, delinquency, fines) must have documented proof of delivery for potential legal use.
...
The reservation of common areas is a right of the current (non-delinquent) condominium owner, governed by internal regulations. No overlapping reservations are permitted. Internal regulations define: maximum reservations per unit per month, minimum advance booking, cancellation notice periods, and any fees or deposits.
...
```
Resident requests a reservation (area, date, time)
       ↓
Check: availability + delinquency status + reservation limit per unit
       ↓
   [Blocked] → Notification of reason (unavailable / delinquent / limit reached)
       ↓
   [Available] → Reservation confirmed → Added to shared calendar
       ↓
Automatic reminder (e.g., 24h and 2h before)
       ↓
After use: optional checkout checklist + area released for next reservations
```

#### Domain Rules for Reservations

- Delinquent owners may not make new reservations of common areas (bylaw right restricted to current owners).
- The reserved area must be returned in the same condition; non-compliance results in a cleaning fee or damage charge per internal regulations.
- Late cancellation or no-show may trigger a temporary block on new reservations as defined in the internal regulations.
- Reservations on holidays or weekends may have distinct rules (additional fees, reduced hours) as defined by the bylaws.
- Guest access to reserved areas requires prior authorization from the reserving owner.
...
An incident is any formal report by a resident about a problem, infraction, irregularity, or request requiring a response or action from management. Formal incident tracking is essential for management transparency and protects both the resident and the syndic in potential legal proceedings.
...
Resident files incident (app / email / in-person)
       ↓
Protocol number generated + syndic/management notified
       ↓
Responsible party analyzes and categorizes
   ...
   [Suggestion]      → Logged for assembly agenda
       ↓
Status updates communicated to the complainant
       ↓
Resolution → Syndic records closure with description
       ↓
Resident may confirm resolution or reopen
       ↓
...
> These are best-practice references; internal regulations may define their own SLAs.
...
Polls are non-binding consultations held by the syndic or administration to gather residents' opinions on topics of collective interest. They differ from **assembly votes** (which have legal deliberative force) in that they are listening and planning instruments. Results generate no legal obligations.

#### Typical Purposes

- Gather preferences before proposing an agenda item at assembly (e.g., "remote or on-site gatehouse?")
- Assess satisfaction with contracted services (cleaning, security, management company)
- Decide everyday topics without requiring a formal assembly
- Prioritize works or improvements when options and budget are limited
...
Syndic creates poll (title, question, type, options, deadline, target audience)
       ↓
Poll published + residents notified
       ↓
Residents respond within deadline
       ↓
Results compiled after deadline
       ↓
Syndic reviews results and decides next step
       ↓
Results shared with all residents (transparency)
```

#### Domain Rules for Polls

- Each unit (not each person) has one vote per poll, consistent with the condominium voting principle.
- Polls are non-binding and cannot substitute a formal assembly vote for matters that legally require a quorum.
- Results should be published to all residents after closing for transparency.
- Delinquent owners may participate in polls (unlike formal assemblies, where they lose voting rights).

---

### 9.5 Packages and Delivery

- Package management: receipt protocol by doorman, resident notification, defined pickup deadline.
- Growing trend: smart lockers for autonomous 24h pickup without doorman intervention.
- Delivery persons: access to gatehouse area only; no direct access to floors without resident authorization.
...
## 11. Condominium Asset Management

### 11.1 Concept and Legal Responsibility
...
### 11.3 Inventory and Asset Tagging
...
Condition review                               │ loop
  ...
  Sale of relevant value → assembly required
       ↓              ↓              ↓
    Repair         Disposal       Sale / Donation
    (WO)        (documented)   (assembly approval)
                     ↓              ↓
               Asset write-off with record
```

### 11.5 Asset Depreciation
...
### 12.1 Distinction Between Assets and Consumables

| Dimension | Asset (Section 11) | Consumable (This Section) |
|---|---|---|
...
Product registration (SKU, category, unit, minimum stock)
                    ↓
         Stock entry (purchase, donation, transfer)
                    ↓
         Use / Exit recorded (by employee or linked to Work Order)
                    ↓
         Balance updated
                    ↓
    Balance ≤ minimum stock? → Yes → Replenishment alert
                    ↓
    Purchase request (manual or auto-generated)
                    ↓
    Approval (syndic / superintendent)
                    ↓
    Purchase completed → Invoice recorded → Stock entry
```

### 12.4 Minimum Stock and Reorder Point

- **Minimum stock:** quantity below which operational disruption is risked. Triggers a replenishment alert.
...
- **Average monthly consumption:** calculated based on the exit history of the last 3–6 months.
...
- Reorder point = (2 × 3) + 5 = **11 bottles**.

### 12.5 Item Requests and Exits
...
- Alert: items approaching expiry (recommended: 30 days in advance).
- FIFO policy (*First In, First Out*): older items are consumed first.
- Expired items must be written off from stock with a disposal record.

### 12.7 Consumable Supplier Management

- Each product may have one or more registered suppliers, with a reference price and lead time.
- Purchase history enables identification of price variations and supports negotiations.
...
### 12.8 Reports and Stock Intelligence
