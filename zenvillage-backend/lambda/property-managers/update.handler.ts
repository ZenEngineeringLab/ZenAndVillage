...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    ...
    await propertyManagerRepository.update(tenantId, id, body)
    const updated = await propertyManagerRepository.getById(tenantId, id)
    metrics.addMetric('PropertyManagerUpdated', MetricUnit.Count, 1)
    return ok(updated)
  } catch (err) {
    logger.error('Failed to update property manager', { err, id, requestId })
    return internalError('Failed to update property manager', requestId)
  }
}
