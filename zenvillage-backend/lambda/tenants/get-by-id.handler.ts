...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    const tenant = await tenantRepository.getById(tenantId, id)
    if (!tenant) return notFound('Tenant', id, requestId)
    return ok(tenant)
  } catch (err) {
    logger.error('Failed to get tenant', { err, id, requestId })
    return internalError('Failed to retrieve tenant', requestId)
  }
}
