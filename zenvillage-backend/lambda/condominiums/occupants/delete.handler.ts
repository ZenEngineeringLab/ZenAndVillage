...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    await occupantRepository.delete(tenantId, condoId, unitId, occupantId)
    return noContent()
  } catch (err: any) {
    if (err?.name === 'ConditionalCheckFailedException') return notFound('OccupantTenant', occupantId, requestId)
    logger.error('Failed to delete occupant', { err, requestId })
    return internalError('Failed to delete occupant', requestId)
  }
}
