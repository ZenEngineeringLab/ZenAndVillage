...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    ...
    await condominiumRepository.update(tenantId, id, body)
    const updated = await condominiumRepository.getById(tenantId, id)
    metrics.addMetric('CondominiumUpdated', MetricUnit.Count, 1)
    return ok(updated)
  } catch (err) {
    logger.error('Failed to update condominium', { err, id, requestId })
    return internalError('Failed to update condominium', requestId)
  }
}
