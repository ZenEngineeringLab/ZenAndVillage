...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    await ownerRepository.delete(tenantId, condoId, unitId, ownerId)
    return noContent()
  } catch (err: any) {
    if (err?.name === 'ConditionalCheckFailedException') return notFound('Owner', ownerId, requestId)
    logger.error('Failed to delete owner', { err, requestId })
    return internalError('Failed to delete owner', requestId)
  }
}
