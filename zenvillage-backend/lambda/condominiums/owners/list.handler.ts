...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    const owners = await ownerRepository.list(tenantId, condoId, unitId)
    return ok(owners)
  } catch (err) {
    logger.error('Failed to list owners', { err, requestId })
    return internalError('Failed to list owners', requestId)
  }
}
