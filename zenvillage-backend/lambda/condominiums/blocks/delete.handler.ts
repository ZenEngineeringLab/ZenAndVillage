...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    await blockRepository.delete(tenantId, condoId, blockId)
    return noContent()
  } catch (err: any) {
    if (err?.name === 'ConditionalCheckFailedException') return notFound('Block', blockId, requestId)
    logger.error('Failed to delete block', { err, requestId })
    return internalError('Failed to delete block', requestId)
  }
}
