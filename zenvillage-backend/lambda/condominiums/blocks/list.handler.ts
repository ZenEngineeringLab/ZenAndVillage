...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  try {
    const blocks = await blockRepository.list(tenantId, condoId)
    return ok(blocks)
  } catch (err) {
    logger.error('Failed to list blocks', { err, requestId })
    return internalError('Failed to list blocks', requestId)
  }
}
