...
const mainHandler = async (event: APIGatewayProxyEventV2WithLambdaAuthorizer<any>) => {
  ...
  if (!condoId || !blockId) return notFound('Block', 'undefined', requestId)

  const patch = (event as any).body ?? {}

  try {
    await blockRepository.update(tenantId, condoId, blockId, patch)
    return ok({ id: blockId })
  } catch (err: any) {
    if (err?.name === 'ConditionalCheckFailedException') return notFound('Block', blockId, requestId)
    logger.error('Failed to update block', { err, requestId })
    return internalError('Failed to update block', requestId)
  }
}
