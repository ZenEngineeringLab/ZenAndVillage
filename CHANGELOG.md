...
### Changed
- **README staging guide updated for smoke testing** — Phase 8 now documents the full seeded dataset (plan catalog, 4 tenants, platform admin, pending owner, pending subscription) and the `--clean` flag; Phase 9 splits smoke tests into a tenant-admin flow and a platform-admin approval flow with the seeded credentials; the stack list and backend env-var reference now include the plans and subscriptions stacks
